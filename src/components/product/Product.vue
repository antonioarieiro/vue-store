<template>
  <div class="w-full rounded overflow-hidden shadow-lg mb-4 " @click="changeRouter(product.id)">
    ...
    <div class="flex justify-center flex-wrap wrap">
      <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">#{{
        product.category
      }}</span>
      <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">R${{
        product.price
      }}</span>
    </div>
